...
from utils.preprocessing import load_img,load_gray_img, get_bbox, process_bbox, generate_patch_image, augmentation, compute_iou, augmentation_together,augmentation_triple
...
class MuCo(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.img_dir = osp.join('/database/changhee', 'MuCo', 'data')
        self.SMPL_edge_overlap_dir = osp.join('/database/changhee', 'MuCo', 'SMPL_overlap_edge')
        self.annot_path = osp.join('/database/changhee', 'MuCo', 'data', 'MuCo-3DHP.json')
        self.smpl_param_path = osp.join('/database/changhee', 'MuCo', 'data', 'smpl_param.json')
        self.fitting_thr = 25 # milimeter
        self.resize_transform=T.Resize(size=(64,64))
        if cfg.human_parsing==True:
            self.parsing_path="/database/changhee/MuCo/densepose_annot"
    ...
    def load_data(self):
        ...
        for iid in db.imgs.keys():
            ...
            aug_set,number,img_names=imgname.split("/")
            try:
                parsing_bbox=parsing_bbox_data[aug_set][number][img_names]
            except:
                parsing_bbox=[]
            SMPL_edge_overlap_dir=osp.join(self.SMPL_edge_overlap_dir,imgname)
            focal = img["f"]
            princpt = img["c"]
            ...
            for pid in pid_list:
                ...
                if cfg.SMPL_overlap==True:
                    datalist.append({
                        'img_path': img_path,
                        'SMPL_edge_overlap_dir':SMPL_edge_overlap_dir,
                ...
                if cfg.human_parsing==True:
                    ...
                    datalist.append({
                        'img_path': img_path,
                        'SMPL_edge_overlap_dir':SMPL_edge_overlap_dir,
                        #'SMPL_edge_occ_dir':SMPL_edge_occ_dir,
                        'img_shape': (img_height, img_width),
                        'bbox': bbox,
                        'tight_bbox': tight_bbox,
                        'joint_img': joint_img,
    ...
    def __getitem__(self, idx):
        ...
        if cfg.SMPL_overlap==True:
            img_path, img_shape, bbox, smpl_param, cam_param = data['img_path'],data['img_shape'], data['bbox'], data['smpl_param'], data['cam_param']
            SMPL_edge_overlap_dir=data['SMPL_edge_overlap_dir']
        
        #print(SMPL_edge_path)
            img=load_img(img_path)
            if os.path.exists(SMPL_edge_overlap_dir):
                SMPL_edge=load_gray_img(SMPL_edge_overlap_dir)
            else:
                print(SMPL_edge_overlap_dir," is not exist")
        ...
        if cfg.SMPL_edge==True:
            SMPL_edge=self.transform(SMPL_edge.astype(np.float32))
            SMPL_edge=SMPL_edge/255.
        #cv2.imwrite("/home/qazw5741/3DCrowdNet_RELEASE/test_img.jpg",img)
        ...
        # check truncation
        edge_module_joint_valid=muco_joint_valid.copy()
        ...
        edge_module_joint_valid= transform_joint_to_other_db(edge_module_joint_valid, self.muco_joints_name, self.joints_name)
        ...
        cv2.waitKey(1)
        """
        edge_module_joint=input_muco_joint_img.copy()
        edge_module_joint = transform_joint_to_other_db(edge_module_joint, self.muco_joints_name, self.joints_name)
        ...
        if cfg.distillation_module==True:
            inputs = {'img': img,'test_img':test_img,'SMPL_edge':SMPL_edge, 'joints': input_muco_joint_img[:, :2], 'joints_mask': muco_joint_trunc}
        elif cfg.SMPL_edge==True and cfg.distillation_module==False:
            if cfg.SMPL_overlap==True:
                inputs = {'img': img,'smpl_edge':SMPL_edge,'joints': input_muco_joint_img[:, :2], 'joints_mask': muco_joint_trunc}
            else:
                inputs = {'img': img,'smpl_edge':SMPL_edge,'joints': input_muco_joint_img[:, :2], 'joints_mask': muco_joint_trunc}
        elif cfg.edge_module==True:
            inputs ={'img': img,'smpl_edge':SMPL_edge,'edge_module_input_joint':edge_module_joint,"edge_module_input_joint_valid":edge_module_joint_valid,'resize_img':resize_img,'joints': input_muco_joint_img[:, :2], 'joints_mask': muco_joint_trunc}
        elif cfg.nothing==True:
            inputs = {'img': img,'test_img':test_img,'resize_img':resize_img,'joints': input_muco_joint_img[:, :2], 'joints_mask': muco_joint_trunc}
        elif cfg.human_parsing==True:
            inputs = {'img': img,'parsing':parsing_data,'joints': input_muco_joint_img[:, :2], 'joints_mask': muco_joint_trunc}
