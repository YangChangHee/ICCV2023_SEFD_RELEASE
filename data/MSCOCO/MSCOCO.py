...
from utils.preprocessing import load_img,load_gray_img, process_bbox, augmentation, compute_iou, augmentation_together, augmentation_triple
...
class MSCOCO(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.img_path = osp.join('/database/changhee',  'MSCOCO', 'images')
        
        self.annot_path = osp.join('/database/changhee',  'MSCOCO', 'annotations')
        self.cliff_smpl=osp.join('/database/changhee/MSCOCO','convert_json_pseudo_Gt_mscoco.json')
        self.rootnet_output_path = osp.join('/database/changhee',  'MSCOCO', 'rootnet_output', 'bbox_root_coco_output.json')
        self.fitting_thr = 3.0 # pixel in cfg.output_hm_shape space
        if cfg.human_parsing==True:
            self.parsing_path="/database/changhee/MSCOCO/images/densepose_annot"
    ...
    def load_data(self):
        ...
        if cfg.cliff==False:
            ...
            if self.data_split == 'train':
                for iid in db.imgs.keys():
                    ...
                    for aid in aids:
                        ...
                        SMPL_edge_path=self.img_path+'/SMPL_overlap_edge/'+img['file_name']
                ...
                for aid in db.anns.keys():
                    ...
                    smpl_param = None
                    datalist.append({'img_path': img_path,'SMPL_edge_path':SMPL_edge_path, 'img_shape': (height, width), 'bbox': bbox, 'tight_bbox': tight_bbox, 'joint_img': joint_img, 'joint_valid': joint_valid, 'smpl_param': smpl_param})
                    if len(datalist) > 100:
                        break
            ...
            with open(osp.join(self.cliff_smpl)) as f:
            # with open(osp.join(self.annot_path, 'coco_smplifyx_train.json')) as f:
                smpl_params = json.load(f)
            ...
            if self.data_split == 'train':
                for iid in db.imgs.keys():
                    ...
                    for aid in aids:
                        ...
                        if str(aid) in smpl_params:
                            smpl_param = smpl_params[str(aid)]
                            #smpl_param['princpt']=np.array([width/2, height/2])
                            if int(smpl_param['is_smpl'])!=1:
                                smpl_param = None
                        else:
                            smpl_param = None
    ...
    def __getitem__(self, idx):
        ...
        if cfg.SMPL_edge==True:
            ...
            img=load_img(img_path)
            img,SMPL_edge, img2bb_trans, bb2img_trans, rot, do_flip=augmentation_together(img,SMPL_edge,bbox,self.data_split)
            #pidinet_img=self.transform1(img.copy())
        ...
        if self.data_split == 'train':
            ...
            # check truncation
            edge_module_joint_valid=coco_joint_valid.copy()
            ...
            edge_module_joint_valid = transform_joint_to_other_db(edge_module_joint_valid, self.coco_joints_name, self.joints_name)
            ...
            """
            edge_module_joint=input_coco_joint_img.copy()
            edge_module_joint = transform_joint_to_other_db(edge_module_joint, self.coco_joints_name, self.joints_name)
            ...
            if smpl_param is not None:
                ...
                if cfg.cliff==False:
                    ...
                    cam_param_cliff={
                        'focal': [smpl_param['cam_focal'],smpl_param['cam_focal']],
                        'princpt':smpl_param['princpt']
                    }
                    smpl_param, cam_param = smpl_param, cam_param_cliff
                # 여기서 기존의 Cam_param확인
            ...
            if cfg.distillation_module==True:
                inputs = {'img': img,'test_img':test_img,'SMPL_edge':SMPL_edge, 'joints': input_coco_joint_img[:, :2], 'joints_mask': joint_mask}
            elif cfg.SMPL_edge==True and cfg.distillation_module==False:
                if cfg.SMPL_single==True:
                    inputs = {'img': img,'smpl_edge':SMPL_edge,'resize_img':resize_img,'joints': input_coco_joint_img[:, :2], 'joints_mask': joint_mask}
                elif cfg.SMPL_overlap==True:
                    inputs = {'img': img,'smpl_edge':SMPL_edge,'joints': input_coco_joint_img[:, :2], 'joints_mask': joint_mask}
                elif cfg.SMPL_Multi==True:
                    inputs = {'img': img,'smpl_edge':SMPL_edge, 'joints': input_coco_joint_img[:, :2], 'joints_mask': joint_mask}
            elif cfg.edge_module==True:
                inputs ={'img': img,'smpl_edge':SMPL_edge,'edge_module_input_joint':edge_module_joint,"edge_module_input_joint_valid":edge_module_joint_valid,'resize_img':resize_img, 'joints': input_coco_joint_img[:, :2], 'joints_mask': joint_mask}
            elif cfg.nothing==True:
                inputs = {'img': img,'test_img':test_img,'resize_img':resize_img, 'joints': input_coco_joint_img[:, :2], 'joints_mask': joint_mask}
            elif cfg.human_parsing==True:
                inputs = {'img': img,'parsing':parsing_data,'joints': input_coco_joint_img[:, :2], 'joints_mask': joint_mask}
