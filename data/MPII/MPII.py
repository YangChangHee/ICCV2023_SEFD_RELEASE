...
from utils.preprocessing import compute_iou, process_bbox, load_img,load_gray_img, augmentation, augmentation_together, augmentation_triple
...
class MPII(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.img_path = osp.join('/database/changhee/MPII',)
        self.SMPL_edge_path = osp.join('/database/changhee/MPII','SMPL_overlap_edge')
        self.annot_path = osp.join('/database/changhee/MPII', 'annotations')
        ...
        if cfg.human_parsing==True:
            self.parsing_path="/database/changhee/MPII/densepose_annot"
    ...
    def load_data(self):
        ...
        if cfg.human_parsing==True:
            with open(osp.join(self.img_path,"human_parsing_bbox.json"),"r") as f:
                parsing_bbox_data=json.load(f)
        if cfg.cliff==False:
            with open(osp.join(self.annot_path, 'MPII_train_SMPL_NeuralAnnot.json')) as f:
                smpl_params = json.load(f)
        else:
            with open(osp.join(self.annot_path, 'convert_json_pseudo_Gt_mpii.json')) as f:
                smpl_params = json.load(f)
        datalist = []
        for iid in db.imgs.keys():
            ...
            for aid in aids:
                ...
                try:
                    parsing_bbox=parsing_bbox_data[img['file_name'].split("/")[-1][:-4]]
                except:
                    parsing_bbox=[]
                imgname=img['file_name'].split("/")[-1]
                SMPL_edge_path=osp.join(self.SMPL_edge_path,imgname)
                ...
                if str(aid) in smpl_params:
                    ...
                    if cfg.cliff==True:
                        smpl_param['cam_param']['princpt']=np.array([width/2, height/2])
                else:
                    smpl_param = None
                if cfg.human_parsing==True:
                    list_bbox_parsing=[]
                ...
                if cfg.human_parsing==False:
                    tmplist.append({'img_path': img_path,'SMPL_edge_path':SMPL_edge_path, 'img_shape': (height, width), 'bbox': bbox, 'tight_bbox': tight_bbox, 'joint_img': joint_img, 'joint_valid': joint_valid, 'smpl_param': smpl_param})
                else:
                    tmplist.append({'parsing_bbox':parsing_index,'bbox_p':bbox_p,'img_path': img_path,'SMPL_edge_path':SMPL_edge_path, 'img_shape': (height, width), 'bbox': bbox, 'tight_bbox': tight_bbox, 'joint_img': joint_img, 'joint_valid': joint_valid, 'smpl_param': smpl_param})
    ...
    def __getitem__(self, idx):
        ...
        img_path,SMPL_edge_path, img_shape, bbox = data['img_path'],data['SMPL_edge_path'], data['img_shape'], data['bbox']
        ...
        #print(img.shape)
            ...
            SMPL_edge=SMPL_edge/255.
                ...
                if not bbox_index==100000:
                    parsing_img_path=img_path.split("/")[-1][:-4]
                    parsing_img=cv2.imread(osp.join(self.parsing_path,"seg_"+parsing_img_path+"_{}.jpg".format(bbox_index)),cv2.IMREAD_GRAYSCALE)
        ...
        if cfg.SMPL_edge==True:
            SMPL_edge=self.transform(SMPL_edge.astype(np.float32))
            SMPL_edge=SMPL_edge/255.
            #if cfg.SMPL_Multi==True:
            #    SMPL_edge_occ=self.transform(SMPL_edge_occ.astype(np.float32))
        if cfg.human_parsing==True:
            img = self.transform(img.astype(np.float32))
            resize_img=self.resize_transform(img)
            img=img/255.
        ...
        edge_module_joint_valid=mpii_joint_valid.copy()
        # check truncation
        ...
        edge_module_joint_valid = transform_joint_to_other_db(edge_module_joint_valid, self.mpii_joints_name, self.joints_name)
        ...
        """
        edge_module_joint=input_mpii_joint_img.copy()
        edge_module_joint = transform_joint_to_other_db(edge_module_joint, self.mpii_joints_name, self.joints_name)
        ...
        if cfg.EPS_plus==True or cfg.EPS_gray ==True:
            inputs = {'img': img,'resize_img':resize_img, 'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
        if cfg.distillation_module==True:
            inputs = {'img': img,'test_img':test_img,'SMPL_edge':SMPL_edge, 'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
        elif cfg.SMPL_edge==True and cfg.distillation_module==False:
            if cfg.SMPL_single==True:
                inputs = {'img': img,'smpl_edge':SMPL_edge, 'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
            elif cfg.SMPL_overlap==True:
                inputs = {'img': img,'smpl_edge':SMPL_edge,'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
            elif cfg.SMPL_Multi==True:
                #print(img.shape)
                inputs = {'img': img,'smpl_edge':SMPL_edge,'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
        elif cfg.edge_module==True:
            inputs ={'img': img,'smpl_edge':SMPL_edge,'edge_module_input_joint':edge_module_joint,"edge_module_input_joint_valid":edge_module_joint_valid,'resize_img':resize_img, 'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
        elif cfg.nothing==True:
            inputs = {'img': img,'test_img':test_img,'resize_img':resize_img, 'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
        elif cfg.human_parsing==True:
            inputs = {'img': img,'parsing':parsing_data,'joints': input_mpii_joint_img[:, :2], 'joints_mask': mpii_joint_trunc}
