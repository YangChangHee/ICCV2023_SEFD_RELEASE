...
# Putting Path Data Directory(MSCOCO)
...
def multi_make_mscoco_smpl_Edge(data_path):
    data_path=osp.join(data_path,"MSCOCO")
    ced=canny_edge()
    if not os.path.exists(data_path+'/images/SMPL_overlap_edge'):
        os.makedirs(data_path+'/images/SMPL_overlap_edge')
        print("make of directory : "+data_path+'/images/SMPL_overlap_edge')

    if not os.path.exists(data_path+'/images/SMPL_all_edge'):
        os.makedirs(data_path+'/images/SMPL_all_edge')
        print("make of directory : "+data_path+'/images/SMPL_all_edge')
    ...
    for iid in tqdm(db.imgs.keys()):
        ...
        list_camera_parma=[]
        #### 여기 ####
#        if data_path+'/images/SMPL_multi_edge/'+'SMPL_multi_edge_'+image_file_name:

        if not os.path.exists(data_path+'/images/SMPL_multi/'+'SMPL_multi_'+image_file_name):
            ...
            if length >=1:
                ...
                cv2.imwrite(data_path+'/images/SMPL_all_edge/'+'SMPL_all_edge_'+image_file_name,tt1[1][0][0].cpu().numpy()*255)
                cv2.imwrite(data_path+'/images/SMPL_overlap_edge/'+'SMPL_overlap_edge_'+image_file_name,result1*255)
            else:
                pass
            

if __name__ =="__main__":
    multi_make_mscoco_smpl_Edge(data_path)