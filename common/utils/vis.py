...
def smpl_edge_render_mesh(img, mesh, face, cam_param, color=(1.0, 1.0, 0.9, 1.0)):
    ...
    # light
    # color=[1.0, 1.0, 1.0]
    light = pyrender.DirectionalLight(color=[0.8, 0.8, 0.8], intensity=0.8)
    light_pose = np.eye(4)
    light_pose[:3, 3] = np.array([0, -1, 1])
    scene.add(light, pose=light_pose)
    light_pose[:3, 3] = np.array([0, 1, 1])
...
def multi_render_mesh(img, mesh_list, face, cam_param,color=(1.0, 1.0, 0.9, 1.0)):
    # mesh
    trimesh_list=[]
    for i in mesh_list:
        trimesh_list.append(trimesh.Trimesh(i, face))
    rot = trimesh.transformations.rotation_matrix(np.radians(180), [1, 0, 0])

    for i in trimesh_list:
        i.apply_transform(rot)
    
    material = pyrender.MetallicRoughnessMaterial(metallicFactor=0.0, alphaMode='OPAQUE', baseColorFactor=color)

    for n,i in enumerate(trimesh_list):
        trimesh_list[n] = pyrender.Mesh.from_trimesh(i, material=material, smooth=False)
...
def new_multi_render_mesh(img, mesh_list, face, cam_param,color=(1.0, 1.0, 0.9, 1.0)):
    # mesh
    scene1 = pyrender.Scene(bg_color=[0.0, 0.0, 0.0, 0.0], ambient_light=(0.3, 0.3, 0.3))
    for i,j in zip(mesh_list,cam_param):
        mesh=trimesh.Trimesh(i, face)
        rot = trimesh.transformations.rotation_matrix(np.radians(180), [1, 0, 0])
        mesh.apply_transform(rot)
        material = pyrender.MetallicRoughnessMaterial(metallicFactor=0.0, alphaMode='OPAQUE', baseColorFactor=color)
        mesh = pyrender.Mesh.from_trimesh(mesh, material=material, smooth=False)
        scene = pyrender.Scene(bg_color=[0.0, 0.0, 0.0, 0.0], ambient_light=(0.3, 0.3, 0.3))
        scene.add(mesh, 'mesh')
        focal, princpt = j['focal'], j['princpt']
        camera = pyrender.IntrinsicsCamera(fx=focal[0], fy=focal[1], cx=princpt[0], cy=princpt[1])
        scene.add(camera)
    renderer = pyrender.OffscreenRenderer(viewport_width=img.shape[1], viewport_height=img.shape[0], point_size=1.0)
    # renderer
...
def mscoco_multi_render_mesh(img, mesh_list, face, cam_params,color=(1.0, 1.0, 0.9, 1.0)):
    ...
    focal, princpt = cam_params[-1]['focal'], cam_params[-1]['princpt']
    camera = pyrender.IntrinsicsCamera(fx=focal[0], fy=focal[1], cx=princpt[0], cy=princpt[1])
    scene.add(camera)
...
def h36m_render_mesh(img, mesh, face, cam_param, color=(1.0, 1.0, 0.9, 1.0)):
    #color=(1.0, 1.0, 0.9, 1.0)
    # mesh
    
    #t_focla,t_princpt=cam_param['focal'], cam_param['princpt']
    #mesh[:,0]=(mesh[:,0]-t_princpt[0])/t_focla[0]*mesh[:,2]
    #mesh[:,1]=(mesh[:,1]-t_princpt[1])/t_focla[1]*mesh[:,2]
