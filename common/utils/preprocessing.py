...
def load_gray_img(path, order='gray'):
...
def augmentation_together(img,EPS,bbox,data_split,exclude_flip=False):
    ...
    if cfg.EPS_gray==True:
        EPS,_,_=generate_patch_image(EPS, bbox, scale, rot, do_flip, cfg.input_img_shape,gray=True)
    elif cfg.SMPL_edge==True:
        EPS,_,_=generate_patch_image(EPS, bbox, scale, rot, do_flip, cfg.input_img_shape,gray=True)
    elif cfg.edge_module==True:
        EPS,_,_=generate_patch_image(EPS, bbox, scale, rot, do_flip, cfg.input_img_shape,gray=True)
    else:
        EPS,_,_=generate_patch_image(EPS, bbox, scale, rot, do_flip, cfg.input_img_shape)
    ...
    if cfg.EPS_gray==False and cfg.EPS_plus==True:
...
def augmentation_triple(img,edge,occ,bbox,data_split,exclude_flip=False):
    if data_split == 'train':
        scale, rot, color_scale, do_flip = get_aug_config(exclude_flip,)
    else:
        scale, rot, color_scale, do_flip = 1.0, 0.0, np.array([1,1,1]), False
    ...
    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    edge,_,_=generate_patch_image(edge, bbox, scale, rot, do_flip, cfg.input_img_shape,gray=True)
    occ,_,_=generate_patch_image(occ, bbox, scale, rot, do_flip, cfg.input_img_shape,gray=True)
    ...
    img = np.clip(img * color_scale[None,None,:], 0, 255)
    return img,edge,occ, trans, inv_trans, rot, do_flip

def generate_patch_image(cvimg, bbox, scale, rot, do_flip, out_shape,gray=False):
    img = cvimg.copy()
